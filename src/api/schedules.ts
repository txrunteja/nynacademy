...
export async function getTodaySchedules() {
  const today = formatInIST(new Date(), "yyyy-MM-dd");
  const { data, error } = await supabase
    .from("schedules")
    .select("*, faculty:faculty_id(id,name), student:student_id(id,name), batch:batch_id(id,name)")
    .gte("start_time", `${today}T00:00:00+05:30`)
    .lte("start_time", `${today}T23:59:59+05:30`)
...
export async function getSchedulesByDay(date: string) {
  const { data, error } = await supabase
    .from("schedules")
    .select("*")
    .gte("start_time", `${date}T00:00:00+05:30`)
    .lte("start_time", `${date}T23:59:59+05:30`)
    .order("start_time", { ascending: true });
  if (error) throw error;
...
export async function getSchedulesByRange(startDate: string, endDate: string) {
  const { data, error } = await supabase
    .from("schedules")
    .select("*")
    .gte("start_time", `${startDate}T00:00:00+05:30`)
    .lte("start_time", `${endDate}T23:59:59+05:30`)
    .order("start_time", { ascending: true });
  if (error) throw error;
  return (data ?? []) as Schedule[];
}
...
export async function markSchedule(
  ...
  const attendancePayload: Omit<AttendanceLog, "id" | "created_at"> = {
    ...
    date: formatInIST(new Date(), "yyyy-MM-dd"),
    status,
    notes: note,
  };
...
/**
 * Fetches past schedules for the history log.
 * Shows a maximum of 10 schedules, with optional search filter by subject.
 */
export async function getPastSchedules(search: string = "") {
  const now = new Date().toISOString();
  let query = supabase
    .from("schedules")
    .select("*, faculty:faculty_id(id,name), student:student_id(id,name), batch:batch_id(id,name)")
